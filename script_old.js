...
function computePercentileBands(sims, labelBase = "Current", color="#0275d8", bandColor="rgba(2,117,216,0.15)") {
  ...
  // Helper to compute percentiles from sorted array
...
function scenarioAssessment(sims, percentileDatasets) {
  ...
  // Find first year where the 10th percentile drops below zero
  const tenthPercentileCurve = percentileDatasets[0].data; // Assuming this is 10th percentile
  const depletionYear = tenthPercentileCurve.findIndex(v => v <= 0);

  if (depletionYear >= 0) {
    message += ` Based on the 10th percentile curve, your savings could be depleted by year ${depletionYear + 1}.`;
  }
